...
public class PlayerController : MonoBehaviour {
    ...
    Vector2 currentPlatformPosition;    // position of the current platform
    ...
    [HideInInspector]
    public Coroutine lastRoutine;      // variable to stop coroutines
    ...
    void Start() {
        ...
        currentPlatformPosition.x = currentPlatform.transform.position.x;
        currentPlatformPosition.y = currentPlatform.transform.position.y;

        head.transform.position = new Vector3(currentPlatformPosition.x, currentPlatformPosition.y + 1f, -1);
    ...
    public void VictoryHappened() {     // if we collide with our target, we need to make player change the platform
        ...
        PlatformChangingPreparation();

        if (!reverseCoroutineIsOn) {                    // if some other coroutine is not running

            reverseCoroutineIsOn = true;                // while true we can't spawn new parts or change direction
            StartCoroutine(MovePlayerToNextPlatform(part, finishPlatform)); // start moving to the next platform
        }
    }
    ...
    public void ArrivedOnPlatform(GameObject platform)
    {
        PlatformChangingPreparation();
        ...
        {
            reverseCoroutineIsOn = true;                // while true we can't spawn new parts or change direction
            StartCoroutine(MovePlayerToNextPlatform(part, platform)); // start moving to the next platform
        }
    }

    void PlatformChangingPreparation()
    {
        GiveMeFirstPart();
        tempPartIndex = 0;
	...
	IEnumerator MovePlayerToNextPlatform(GameObject part, GameObject nextPlatform) {	// moving player to the next platform (we know part orientation, so we rotate it by 180 and change its position in appropriate way)
		...
		if (tempPartIndex < partIndex) {
			...
			StartCoroutine (MovePlayerToNextPlatform (part, nextPlatform));
		} else {
			
			// unchilding head
			part.transform.DetachChildren ();	// SetActive() works at the end of frame, so we still able to get this part

			//ResetHeadTransform (nextPlatform.transform.position.x, nextPlatform.transform.position.y);
            ResetHeadTransform(nextPlatform.transform.position.x, nextPlatform.transform.position.y);
            currentPlatform = nextPlatform;		// we reached the target, so our next platform becomes our current platform
